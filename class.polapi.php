<?php
	...
	class POLAPI {
		...
		function __construct( $dbaxx, $action ) {
			$config = parse_ini_file(__DIR__ . '/config.ini');
			$this->api_key = $config['balancekey'];
			$this->api_secret = $config['balancesecret'];

/*			$key = $dbaxx->db_select("Select * FROM `polokey` where `poloname` = '" . $user . "'");
			$this->api_key = $key[0]['poloid'];
			$this->api_secret = $key[0]['polosecret'];
*/
		}
		...
		public function get_completebalances() {
			return $this->query(
				array(
		...
		public function move_order($pair, $order_number, $rate, $amount, $action) {
			return $this->query(
				array(
					'command' => 'moveOrder',
					'orderNumber' => $order_number,
		...
		public function get_total_btc_balance() {
			...
			foreach($balances as $coin => $amount){
				...
				if($amount > 0){
					if($coin != "BTC"){
						$tot_btc += (intval($amount) * intval($prices[$pair]));
					}else{
						$tot_btc += intval($amount);
					}
				}
				...
				if($coin != "BTC"){
					...
					foreach($open_orders as $order){
						...
						if($order['type'] == 'buy'){
							$tot_btc += intval($order['total']);
						}elseif($order['type'] == 'sell'){
							$tot_btc += (intval($order['amount']) * intval($prices[$pair]));
						}
						}
					}
				}
